...
(defmethod execute-instructions ((s submarine) instructions)
  (loop for (cmd param) in instructions
        do (funcall (find-function-in-package cmd :advent-of-code)
                    s param)
        finally (return s)))

(defun read-course ()
  (loop for l in (uiop:read-file-lines
                  (asdf:system-relative-pathname 'advent-of-code "inputs/2021/day2"))
